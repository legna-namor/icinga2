		...
		std::vector<Dictionary::Ptr>* runtimeUpdates)
{
	...
	for (auto& dependencyGroup : checkable->GetDependencyGroups()) {
		...
		for (auto it(members.begin()); it != members.end(); /* no increment */) {
			...
			auto edgeStateId(HashValue(new Array{
				dependencyGroup->IsRedundancyGroup() ? redundancyGroupId : dependencyGroup->GetCompositeKey(),
				data->Get("to_node_id"),
			}));
			data->Set("dependency_edge_state_id", edgeStateId);

			String edgeId(HashValue(new Array{data->Get("from_node_id"), data->Get("to_node_id")}));
			hmsetDependencyEdges.emplace_back(edgeId);
			hmsetDependencyEdges.emplace_back(JsonEncode(data));
...
void IcingaDB::UpdateDependenciesState(const Checkable::Ptr& checkable, StateUpdate mode) const
{
	...
	RedisConnection::Queries streamStates, hmsets{{/* dependency states */}, {/* redundancy group states */}};
	...
	for (auto& dependencyGroup : dependencyGroups) {
		...
		for (auto it(members.begin()); it != members.end(); /* no increment */) {
			...
			Dictionary::Ptr stateAttrs(SerializeDependencyEdgeState(dependencyGroup, dep));
			// Find all members that share the same parent starting from the iterator position ("it" inclusively),
			// and merge their relevant state information into a single one.
			...
			std::for_each(++begin, end, [&stateAttrs, &dependencyGroup](const Dependency::Ptr& member) {
				if (stateAttrs->Get("failed") == false) {
					stateAttrs = SerializeDependencyEdgeState(dependencyGroup, member);
				}
			});
			it = end; // Advance the iterator to either end of the container or beginning of the next members block.
			...
			if (mode == StateUpdate::Full) {
				...
				hmsets[0].emplace_back(stateAttrs->Get("id"));
				hmsets[0].emplace_back(JsonEncode(stateAttrs));
			}
		...
		if (isRedundancyGroup) {
			...
			if (mode == StateUpdate::Full) {
				...
				hmsets[0].emplace_back(dependencyGroup->GetIcingaDBIdentifier());
				hmsets[0].emplace_back(JsonEncode(groupSharedState));
				hmsets[1].emplace_back(dependencyGroup->GetIcingaDBIdentifier());
				hmsets[1].emplace_back(JsonEncode(stateAttrs));
			}
	...
	if (!hmsets[0].empty() || !hmsets[1].empty()) {
		if (!hmsets.front().empty()) {
			hmsets[0].insert(hmsets[0].begin(), {"DEL", m_PrefixConfigObject + "dependency:edge:state"});
		}
		if (!hmsets[1].empty()) {
			hmsets[1].insert(hmsets[1].begin(), {"HMSET", m_PrefixConfigObject + "redundancygroup:state"});
		}

		hmsets.erase(
			std::remove_if(hmsets.begin(), hmsets.end(), std::mem_fn(&RedisConnection::Query::empty)),
			hmsets.end()
		);
